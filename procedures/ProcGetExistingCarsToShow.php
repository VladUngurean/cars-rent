...
if (!empty($conn)) {
  ...
  if ($result->num_rows > 0) {
    $data = []; // Initialize $data array before the loop
    while($row = $result->fetch_assoc()) {
      $data[] = array(
        ...
        'carImage' => $row['images'] ?? '', // Handle potential NULL
      );
    }

    if (!empty($data)) {
      echo '<script> let carData = ' . json_encode($data) . '; </script>';
      echo '<script> console.log(carData); </script>';
    }
  } else {
    echo '<script> let carData = null; </script>';
    echo '<script> console.log("All bad"); </script>';
  }

  ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);
  // No need for next_result() if you're not dealing with multiple queries
}
?>